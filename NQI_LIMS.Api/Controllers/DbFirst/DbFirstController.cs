...
namespace NQI_LIMS.Controllers
{
    ...
    public class DbFirstController : ControllerBase
    {
        ...
        public MessageModel<string> GetFrameFiles()
        {
            ...
            if (Env.IsDevelopment())
            {
                BaseDBConfig.MutiConnectionString.Item1.ToList().ForEach(m =>
                {
                    ...
                    data.response += $"库{m.ConnId}-Controller层生成：{FrameSeed.CreateControllers(_sqlSugarClient)} || ";
        ...
        public MessageModel<string> GetFrameFilesByTableNames([FromBody]string[] tableNames, [FromQuery]string ConnID = null)
        {
            ...
            if (Env.IsDevelopment())
            {
                data.response += $"库{ConnID}-CreateControllers：{FrameSeed.CreateControllers(_sqlSugarClient, ConnID, isMuti, tableNames)} || ";
                data.response += $"库{ConnID}-CreateModels：{FrameSeed.CreateModels(_sqlSugarClient, ConnID, isMuti, tableNames)} || ";
